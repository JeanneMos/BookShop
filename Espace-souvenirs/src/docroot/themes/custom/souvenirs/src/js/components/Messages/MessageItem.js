...
const MessageItem = React.memo(function MessageItem({
  message: { body, author, dateTime, picture, email, id, published },
}) {
  ...
  const formattedDate = useMemo(() => {
    if (dateTime) return dateFormat(new Date(dateTime));
  }, [dateTime]);
    ...
    >
      ...
      {!showFullMessage && (
        <ButtonWrapper>
          ...
          >
            Lire la suite
            <span className="sr-only">du message envoyé par {author}</span>
          </Button>
        </ButtonWrapper>
      )}
