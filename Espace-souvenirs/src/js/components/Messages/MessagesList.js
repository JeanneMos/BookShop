...
export default function MessagesList({page}) {
  ...
  useEffect(() => {
    ...
    const sortedByDate = messages?.length > 0 ?messages.sort((a, b) => b.dateTime - a.dateTime) : [];
    const filteredArray = !userState?.isAdmin ? sortedByDate.filter(message => message.published) : sortedByDate;
    const truncatedArray = filteredArray.length > 0 ? filteredArray.slice(0,2) : [];
  ...
  if (messages && messages.length > 8) return <MessagesWithPagination messages={messages} />
