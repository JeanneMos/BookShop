...
export default function Administrator({admin, current}) {
  ...
  const editingPersonalInfo = () => {
    dispatch(administratorPersonalInfoModified({isEditingPersonalInfo: true, isEditingAdministator: false}))
  }
  return (
    <li className='administrator-item'>
      <p className='administrator-info'>
        <span className='administrator-name'>{admin.firstName} {admin.lastName}</span>
