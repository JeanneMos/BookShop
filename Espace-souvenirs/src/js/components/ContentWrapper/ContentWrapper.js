...
const AdministratorsEdit = React.lazy(() => import(/* webpackChunkName: "AdministratorsEdit" */'../ContentPages/AdministratorsEdit'));
...
export default function ContentWrapper() {
  ...
  const tabs = [
    ...
    { name: 'Biographie', id: biographie, hasContent: userState?.isAdmin ? true : biographyState?.isBiographyEdited },
  ...
  const getTabs = () => {
    const arrayOfTabs = getTabsWithContent().map((tab) => {
      const tabInfos = {
        id: tab.id, activeLink: activeTab, handleActiveTab,
      };
      const activeMobileTab = width < mobileBreakpoint ? activeTab : null;
      return (
        <TabItem tabInfos={tabInfos} key={`tab_${tab.id}`} activeMobileTab={activeMobileTab} />
      );
    });
    return arrayOfTabs;
  };
  const handleDisplayedView = () => {
    if ((administratorState?.isEditingAdministator || administratorState?.isEditingPersonalInfo) && userState.isAdmin) {
      return (<React.Suspense fallback={<FullPageLoader />}><AdministratorsEdit /></React.Suspense>) 
    }

    if(!administratorState?.isEditingAdministator && !administratorState?.isEditingPersonalInfo) {
      return ( <ContentPage
        name={activeTab}
