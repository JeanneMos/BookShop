...
export default function HeroBanner({ source, handleModifyHero }) {
  ...
  const getHeroBannerImage = userState.heroBannerImage.name
    ? `${pathToHeroBanners}${userState.heroBannerImage.name}`
    : source;
  const getHeroBannerImageMobile = userState.heroBannerImage.name
    ? `${pathToHeroBannersMobile}${userState.heroBannerImage.name}`
    : source;
