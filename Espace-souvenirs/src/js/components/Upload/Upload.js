...
export default function Upload({
  name, numberOfPhotos, uploadType, closeModal, saveFile, hasImageRequirement=true, dispatchCbAction, btnClass
}) {
  ...
  const handleImagePreview = (files) => {
    ...
    if (files && files.length === 0 && uploadType === 'defuntImage') return <ImagePlaceholder />;
    return null;
  };
  return (
    <>
      ...
      {uploadType === 'defuntImage' ? (
        <div className="upload-defunt-image-wrapper">
          ...
          <div className="modal-image-description">
            <p className="modal-photo-subtitle">Personnaliser l’Espace Souvenirs en y ajoutant une photo du défunt</p>
            <p className="input-file-description">Pour un affichage optimum, privilégier des photos au format portrait</p>
          ...
          <ButtonsWrapper position="right">
            ...
            <Button type="button" btnClass="bg-current" onClickAction={() => saveFile(compressedFiles)}>
